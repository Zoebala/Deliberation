...
class PalmaresController extends Controller
{
    ...
    public function imprimerAnnuel(){

        $classe_id=(int)session("classe_id")[0];

        //recherche du total cours annuel
        $maxima=DB::select("SELECT sum(ponderation*20) as maxima
                            FROM cours
        ...
        $queries=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
                            ...
                            JOIN sections as Sec ON Sec.id=J.section_id
                            JOIN annees as An ON An.id=J.annee_id
                            WHERE Cpon.classe_id=$classe_id
                            Group by nom,postnom,prenom,chef_section,grade,genre,An.lib,Sec.lib
                            Having total=$max");

        //les étudiants ayant suivis tous les cours et ayant obtenu la plus grande distinction
        $PGDS=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
        ...
        //les étudiants ayant suivis tous les cours et ayant obtenu une grande distinction
        $GDS=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
        ...
        //les étudiants ayant suivis tous les cours et ayant obtenu une distinction
        $DS=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
        ...
        //les étudiants ayant suivis tous les cours et ayant obtenu une satisfaction
        $SS=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
        ...
        //les étudiants ayant suivis tous les cours et ayant obtenu un ajournement
        $AS=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
        ...
        //les étudiants ayant suivis tous les cours et ayant obtenu la mention NAF
        $NAFS=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
        ...
        //les assimilés aux ajournés
        $AAA=DB::select("SELECT nom,postnom,prenom,genre,chef_section as chsection,grade as grade,
                sum(C.ponderation*(tj+examenS1)) as coteObtenue,sum(C.ponderation*20) as total,An.lib as annee,Sec.lib as section
                            FROM cours as C
                            JOIN elementcoupons as Elcp ON Elcp.cours_id=C.id
                            JOIN coupons Cpon ON Elcp.coupon_id=Cpon.id
        ...
        if(count($PGDS) > 0 || count($GDS) > 0 || count($DS) > 0 || count($SS) > 0 || count($NAFS) > 0 || count($AAAS) > 0){
            ...
            $pdf = Pdf::loadView('Etats/palmares',$data);
            return $pdf->download('pamares_annuel'.$queries[0]->annee."-".($queries[0]->annee+1).'.pdf');
        }else{
            Notification::make()
            ->title('Aucune donnée trouvée!')
            ->danger()
